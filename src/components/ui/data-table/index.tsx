...
export function DataTable<TData>({
  ...
  const isPaginatedFromApi =
    typeof onPageChange === "function" && typeof currentPage === "number";
  ...
  const [previousTotalResults, setPreviousTotalResults] = useState<number>(
  ...
  const totalPagesValue = isPaginatedFromApi
    ? effectiveTotalPages
    : table.getPageCount();
  const currentPageValue = isPaginatedFromApi
  ...
  const pageNumbers = useMemo(() => {
    ...
    if (currentPageValue < visiblePages - 2) {
      return [
        ...Array.from({ length: visiblePages - 2 }, (_, i) => i),
        "...",
        totalPagesValue - 1,
      ];
    }
    ...
    if (currentPageValue >= totalPagesValue - (visiblePages - 2)) {
      return [
        ...
        ...Array.from(
          ...
          (_, i) => totalPagesValue - (visiblePages - 2) + i
        ),
      ];
    }
    ...
    return [
      ...
      "...",
      ...Array.from(
        { length: visiblePages - 4 },
        (_, i) => currentPageValue - Math.floor((visiblePages - 4) / 2) + i
      ),
      "...",
      totalPagesValue - 1,
    ];
  ...
  const totalR = isPaginatedFromApi
    ? effectiveTotalPages * sizePageValue
    : data.length;
  const totalRecords = isLoading
  ...
  const endResult = isLoading
    ...
    : Math.min(
        startResult +
          (isPaginatedFromApi ? sizePageValue : pagination.pageSize) -
          1,
        totalRecords ?? 0
      );
      ...
      <div className="flex items-center justify-between py-5">
        <div className="relative w-full max-w-lg mr-10">
          <Input
            ...
            className="w-full border border-gray-800 rounded-lg pr-10"
          />
          {globalFilter ? (
            <button
        ...
        {table
          ...
          .some(
            (column) =>
              (column.columnDef as CustomColumnDef<TData>).canHide ?? true
          ) && (
          <div className="relative inline-block text-left">
            <Button
              variant="ghost"
              onClick={() => setIsDropdownOpen((prev) => !prev)}
              className="bg-gray-800/30 border-1 px-2 py-3 text-sm rounded-lg shadow-md shadow-var--primary-50/50 transition-colors duration-200 hover:bg-var--white font-medium text-var--primary-100"
            >
              Columnas
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-lg bg-sidebar shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none max-h-[400px] overflow-y-auto text-sm">
                <div className="py-1">
                  {table
                    .getAllColumns()
                    .filter(
                      (column) =>
                        (column.columnDef as CustomColumnDef<TData>).canHide ??
                        true
                    )
                    .map((column) => (
                      <label
                        key={column.id}
                        className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-800/30"
                      >
                        <input
                          type="checkbox"
      ...
      <Table>
            ...
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header) => {
                ...
                return (
                  <TableHead
                    key={header.id}
                    className="text-left hover:bg-gray-700 bg-gray-800 text-white text-[0.9rem]"
                  >
                    {header.isPlaceholder ? null : canSort ? (
                      <div className="flex items-start justify-between px-4 w-full">
                        ...
                        >
                          ...
                          {isSorted === false && (
                            <ArrowUpDown className="w-5 h-5 opacity-20 flex-shrink-0" />
                          )}
                          {isSorted === "desc" && (
                            <MoveDown className="w-5 h-5 flex-shrink-0" />
                          )}
                          {isSorted === "asc" && (
                            <MoveDown className="w-5 h-5 flex-shrink-0 rotate-180" />
                          )}
                        </button>
                      ...
                      <button className="font-semibold w-full flex justify-left px-4">
                        {flexRender(
                          header.column.columnDef.header,
                          header.getContext()
                        )}
        ...
        <TableBody>
          {isLoading && data.length === 0 ? (
            Array.from({
              length: isPaginatedFromApi ? sizePageValue : pagination.pageSize,
            }).map((_, index) => (
              <SkeletonRow
                key={index}
            ...
            <TableRow>
              <TableCell
                ...
                className="text-center text-xl h-[32.5rem] bg-sidebar text-white"
              >
      ...
      <div className="flex items-center justify-center py-4 relative">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="absolute left-0 -bottom-2 tablet:bottom-8 px-2 py-3 rounded-lg shadow-md shadow-var--primary-50/50 transition-colors duration-200 data-[state=open]:bg-var--white bg-gray-800/30 text-var--primary-100 font-medium text-sm"
            >
          ...
          <DropdownMenuContent
            align="start"
            className="min-w-[8rem] shadow-var--primary-50/50 rounded-lg"
          >
          ...
          <PaginationContent>
            <PaginationItem>
              <PaginationPrevious
                ...
                className={
                  (isPaginatedFromApi && currentPage === 1) ||
                  (!isPaginatedFromApi && !table.getCanPreviousPage())
            ...
            <PaginationItem>
              <PaginationNext
                ...
                className={
                  (isPaginatedFromApi && currentPage === totalPages) ||
                  (!isPaginatedFromApi && !table.getCanNextPage())
                    ? "opacity-20 cursor-not-allowed"
                    : ""
                }
              />
